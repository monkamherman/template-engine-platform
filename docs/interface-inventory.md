...
Sprint 03 has introduced the central route builders, navigation lists, interface registry and generated preview renderer for the V1 skeleton. Public product pages remain branded Sprint 02/early Sprint 03 surfaces. Sprint 06A upgrades `marketing.home` to a page-specific branded landing interface with the approved dark marketing palette, modular hero visual, shop-model tabs, feature grid, offer preview, documentation preview and final CTA. Sprint 06B upgrades `marketing.pricing` to a branded pricing interface with plan cards, license clarification, comparison, decision helper, FAQ and final CTA, while keeping final prices outside the page until approved catalog/checkout data is connected. Sprint 06C upgrades `marketing.features` to a branded features interface with category tabs, shop-model grid, launch workflow, license/service boundary, technical confidence section and final CTA. Sprint 06D upgrades `docs.home` to a branded documentation entry point with search preview, customer paths, category grid, highlighted guides, support bridge, review notice and non-final version card. Sprint 06E-bis upgrades `marketing.faq`, `marketing.contact` and `marketing.about` to branded page-specific public interfaces; Contact remains a disabled preview form until a backend contact provider is approved and connected. Sprint 10A upgrades documentation detail and release pages to branded article interfaces with breadcrumbs, review/version metadata, table of contents, related links, support path and cautious release states. Sprint 07A upgrades `auth.login`, `auth.register`, `auth.forgot-password`, `auth.reset-password` and `auth.verify-email` to branded auth pages. Sprint 07 auth integration connects those pages to Auth.js, encrypted JWT sessions, Prisma-persisted users/accounts/passwords, Google OAuth, email/password and expiring magic-link verification; the interfaces remain below `VALIDATED` until real Google and SMTP credentials are configured and tested. Sprint 08A upgrades `account.dashboard`, `account.orders`, `account.order-detail`, `account.entitlements`, `account.entitlement-detail`, `account.downloads` and `account.releases` to branded fixture/query customer-account pages without exposing real invoices, complete license keys or signed download URLs. Sprint 08B upgrades customer licenses, onboarding, support and settings pages to branded fixture/preview interfaces with masked license keys, disabled sensitive actions, non-persistent forms and explicit storefront-independence copy. Sprint 09A upgrades checkout and payment-state pages to branded preview interfaces without inventing a payment provider or successful payment. Sprint 11A upgrades the first admin commerce block to branded fixture/preview interfaces for operations dashboard, customers, catalogue products/offers/prices, orders and payments. Sprint 11B upgrades the second admin operations block for entitlements, license details, releases, legal review, support, audit and settings with masked secrets, disabled sensitive actions and explicit legal review states. The remaining generated marketing and admin routes are currently `WIREFRAME` unless their runtime registry entry marks a more specific maturity.
...
| `marketing.about` | `/{locale}/about` | About | Explain the project vision, commercial platform boundary and non-promises | content | marketing/product |

Sprint 06E-bis support pages are `BRANDED` public interfaces. FAQ uses categorized shadcn/Radix accordions and repeats the license-service boundary. Contact presents sales, customer support and Pro/Managed request paths with a disabled preview form; no contact message is sent or persisted. About explains the platform vision, problem, solution, operating philosophy and explicit non-promises without fake metrics, testimonials, legal claims or guaranteed outcomes.
...
Sprint 10A documentation detail pages are `BRANDED` content interfaces. They render typed localized content through `modules/content/documents.ts`, expose review status and product-version scope, show local table-of-contents anchors, related links, support/troubleshooting paths and previous/next navigation, and keep release compatibility/changelog information cautious until an approved theme release is connected.
...
| ID | Route | Interface | Main purpose | Initial data mode | Owner |
|---|---|---|---|---|---|
| `checkout.index` | `/{locale}/checkout` | Checkout | Review offer, customer details, legal references and payment readiness | preview | commerce |
| `checkout.success` | `/{locale}/checkout/success` | Payment success state | Explain next steps after verified payment without inventing a real order | preview | commerce/account |
| `checkout.failed` | `/{locale}/checkout/failed` | Payment failed state | Provide retry and support paths without exposing provider errors | preview | commerce/support |

Sprint 09A checkout pages are `BRANDED` preview interfaces. They do not collect card data, do not persist customer fields, do not invent a payment provider or a successful provider event, and keep legal references in draft/review state until approved checkout capture exists.
...
Sprint 08A account core pages are `BRANDED` fixture/query interfaces. They use centralized representative data and explicitly avoid real provider payment references, official invoices, complete license keys and signed download URLs until the corresponding commerce, licensing and release-delivery services are connected. Sprint 08B customer secondary pages are also `BRANDED`: license list/detail pages expose only masked keys and activation states, onboarding/support pages show representative workflow states, and support/settings forms stay disabled preview interfaces until backend persistence, authorization and audit services are connected.
...
Sprint 11A admin commerce pages are `BRANDED` fixture/preview interfaces. They use centralized representative data, mask provider references, disable write/refund/delete/publish actions until backend authorization and audit services are connected, and keep browser-side price/order values out of the commercial source of truth.

Sprint 11B admin operations pages are `BRANDED` fixture/preview interfaces. They use centralized representative data, never expose complete license keys, private keys, peppers, ciphertext, tokens, provider secrets or signed URLs, keep sensitive actions disabled until backend authorization and audit services are connected, show only available release checksums without inventing compatibility claims, and keep draft/review legal documents visibly unapproved.
